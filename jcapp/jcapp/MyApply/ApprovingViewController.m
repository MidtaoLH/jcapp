...
@interface ApprovingViewController (){
...
@implementation ApprovingViewController
NSInteger currentPageCountwait1;
...
- (void)viewDidLoad {
    ...
    currentPageCountwait1=[Common_PageSize intValue];
...
-(void)LoadData
{
    ...
    //设置需要访问的ws和传入参数
    NSString *currentPageCountstr = [NSString stringWithFormat: @"%ld", (long)currentPageCountwait1];
    NSString *strPara = [NSString stringWithFormat:@"AppWebService.asmx/GetPendingInfo?pasgeIndex=%@&pageSize=%@&code=%@&userID=%@&menuID=%@&iosid=%@",@"1",currentPageCountstr,empID,userID,@"31",iosid];
...
- (void)footerClick {
    // 可在此处实现上拉加载时要执行的代码
    // ......
    currentPageCountwait1=currentPageCountwait1+[Common_PageSizeAdd intValue];
    [self LoadData];
    // 模拟延迟3秒
    //[NSThread sleepForTimeInterval:3];
     [NSThread sleepForTimeInterval:0.5];
