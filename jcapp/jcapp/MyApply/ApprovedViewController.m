...
NSInteger currentPageCountwait2;
...
-(void)LoadData
{
    ...
    //设置需要访问的ws和传入参数
    NSString *currentPageCountstr = [NSString stringWithFormat: @"%ld", (long)currentPageCountwait2];
    NSString *strPara = [NSString stringWithFormat:@"AppWebService.asmx/GetPendingInfo?pasgeIndex=%@&pageSize=%@&code=%@&userID=%@&menuID=%@&iosid=%@",@"1",currentPageCountstr,empID,userID,@"32",iosid];
...
- (void)footerClick {
    // 可在此处实现上拉加载时要执行的代码
    // ......
    currentPageCountwait2=currentPageCountwait2+[Common_PageSizeAdd intValue];
    [self LoadData];
    // 模拟延迟3秒
    //[NSThread sleepForTimeInterval:3];
     [NSThread sleepForTimeInterval:0.5];
